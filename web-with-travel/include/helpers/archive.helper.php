 ...
 //获取线路团期
  ...
  public static function getLoadingFile()
  {
   ...
   /**
	...
	//人民币转美元
	public static function getCnyToUsd($price)
	{
		$ch = curl_init();
		$url = 'http://apis.baidu.com/apistore/currencyservice/currency?fromCurrency=CNY&toCurrency=USD&amount='.$price;
		$header = array(
			'apikey: fe23c63a30e4d3f3ee46c96c01530ebd',
		);
		// 添加apikey到header
		...
		return round($res->retData->convertedamount,2);
    ...
    public static function  paySuccess($ordersn,$paySource,$params)
    {
		...
		//支付宝
		if(strpos($paySource, '支付宝')!== false && $arr['typeid']!=2)
		{
			if($params['total_fee']!=$totalPrice)
			{
				exit;
			}
		}
		//微信
		if(strpos($paySource, '微信')!== false && $arr['typeid']!=2)
		{
			$total = $totalPrice * 100;
			if(intval($params['total_fee'])!= $total)
			{
				exit;
			}
		}
		...
		if(strpos($paySource, '贝宝')!== false && $arr['typeid']!=2)
		{
			$rprice = $params['mc_gross'];
			$needPrice = self::getCnyToUsd($totalPrice);
			if($rprice!=$totalPrice)
			{
				exit;
			}
		}
		//银联
		if(strpos($paySource, '银联')!== false && $arr['typeid']!=2)
		{
			$rprice = intval($params['txtAmt']);
		...
		//汇潮
		if(strpos($paySource, '汇潮')!== false && $arr['typeid']!=2)
		{
			$rprice = intval($params['Amount']);
		...
		//快钱
		if(strpos($paySource, '快钱')!== false && $arr['typeid']!=2)
		{
			$payPrice = intval($params['payAmount']);
