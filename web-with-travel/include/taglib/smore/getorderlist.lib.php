...
function getProName($id,$typeid,$productname='')
{
	...
	$sql = "select webid,aid,{$field} as title from {$tablename} where id='$id'";
	$row = $dsql->GetOne($sql);
    $webroot=GetWebURLByWebid($row['webid']);//根据webid获取域名前缀
	$title = !empty($productname) ? $productname : $row['title'];
	$out = "<a href=\"{$webroot}/{$link}/show_{$row['aid']}.html\" target=\"_blank\">{$title}</a>";
	return $out;
