   ...
   //主要参数
    ...
    if ($config['word_type'] != 3)
    {
       ...
       /* $fp = @fopen($config['wordlist_file'], 'rb');
        ...
        $start = strpos($data, "\n", rand(0, 100)) + 1; 
        $end   = strpos($data, "\n", $start); */
        $config_character='abcdefghijklmnopqrstuvwxyz'; 

		$rndstring=$config_character[mt_rand(0,25)].$config_character[mt_rand(0,25)].$config_character[mt_rand(0,25)].$config_character[mt_rand(0,25)];
