...
class AlipayNotify {
	...
	function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			file_put_contents(dirname(__FILE__).'/recevie_result.txt',var_export($_POST,true).PHP_EOL,FILE_APPEND);
			//对notify_data解密
			...
			//logResult($log_text);
			//file_put_contents(dirname(__FILE__).'/return_result.txt',var_export($verify_result,true).PHP_EOL,FILE_APPEND);
			file_put_contents(dirname(__FILE__).'/return_result.txt',$log_text.PHP_EOL,FILE_APPEND);
	...
	function verifyReturn(){
		if(empty($_GET)) {//判断GET来的数组是否为空
			...
			//$responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
			//isSign的结果不是true，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
			file_put_contents(dirname(__FILE__).'/g.txt',var_export($_GET,true));
			if ($isSign) {
				return true;
			} else {
				return false;
