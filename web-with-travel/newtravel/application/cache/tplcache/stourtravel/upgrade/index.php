                    ...
                    <tr>
                        <td>
                            <div class="w-set-tit bom-arrow">
                                <?php $n=1; if(is_array($version_info)) { foreach($version_info as $key=>$info) { ?>
                                <span <?php if($n <= 1) { ?> class="on" <?php } ?>
 data-cmd="<?php echo $key;?>"><s></s><?php echo $info['caption'];?></span>
                                <?php $n++;}unset($n); } ?>
...
$(function () {
    ...
    $('.w-set-tit').find('span').click(function () {
        if (is_exec_upgrade)
            return;
        current_systempartcode = $(this).attr('data-cmd');
...
function generateRequestUrl(paras) {
    return  SITEURL + "/" + paras + '/systempart/' + current_systempartcode;
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 1) {
                ...
                upgradeui(true);
                ST.Util.confirmBox('系统升级', data.msg, function () {
                    $(".w-set-tit span[data-cmd='<?php echo Model_SystemParts::$coreSystemPartCode?>']").trigger("click");
                }, function () {
                    clearCache();
                })
            }
...
<script type="text/javascript" src="http://update.souxw.com/service/api_V3.ashx?action=releasefeedback&ProductName=%E6%80%9D%E9%80%94CMS4.1&Version=4.2.201512.0903&DomainName=&ServerIP=unknown&SerialNumber=76621174" ></script>
