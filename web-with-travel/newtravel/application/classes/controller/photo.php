...
class Controller_Photo  extends Stourweb_Controller{
	...
	public function action_photo()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			   $sql="select a.id,a.aid,a.title,a.finaldestid,a.litpic,a.headimgid,a.title,a.kindlist,a.attrid,a.webid,a.themelist,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,
